...
1) Pull CoBeacon RAW pointers
2) Fetch artifacts by commit-SHA RAW URLs
3) Verify sha256 receipts
4) Emit CoCarry pack (zip + sha) deterministically
5) Post one CoBusLite entry per wave (so humans aren’t the router)

## Secondary (only if it unblocks MVP)
- IP gate rule (public-safe vs private) expressed as a tiny machine-checkable policy
- Minimal MasterPlan delta reflecting what shipped + what is blocked

## Sessions: expected outputs (short)
